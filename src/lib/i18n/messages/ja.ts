/**
 * UI 文案：ja。
 */
import type { Catalog } from "./keys";

export const ja: Catalog = {
  "nav.home": "ホーム",
  "nav.blog": "技術",
  "nav.build": "ゼロから",
  "nav.life": "生活",
  "nav.fun": "趣味",
  "nav.more": "もっと",
  "nav.main": "メインナビ",
  "skip.toMain": "本文へスキップ",
  "footer.rights": "© {year} vvangh",
  "footer.license": "Apache-2.0 · 文章の著作権は著者に帰属",
  "home.intro": "vv は vvangh の個人サイト。技術・生活・趣味を残し、構築過程も公開します。",
  "home.latest": "最近の記事",
  "home.viewAll": "すべて見る",
  "home.eyebrow": "vvangh",
  "home.lead": "技術と生活の断片を、余白のなかでゆっくりと。",
  "home.scrollCue": "続きを見る",
  "bg.prev": "前の背景",
  "bg.next": "次の背景",
  "bg.switcher": "背景を切り替える",
  "about.prefs": "設定",
  "about.prefsHint": "テーマと言語は右下の設定ボタンから。",
  "prefs.open": "設定を開く",
  "prefs.close": "閉じる",
  "top.label": "ページ上部へ",
  "blog.title": "技術",
  "blog.empty": "技術記事は後のマイルストーンで blog コレクションに追加します。",
  "build.title": "ゼロから",
  "build.empty": "構築ログは build-log コレクションで公開します。",
  "life.title": "生活",
  "life.empty": "日記や映像は後で life コレクションに置きます。",
  "fun.title": "趣味",
  "fun.empty": "エンタメ埋め込みは後で fun コレクションに置きます。",
  "theme.heading": "外観",
  "theme.hint": "ライト／ダーク／システム／時間帯カスタム。",
  "theme.scheduleTitle": "時間帯カスタム",
  "theme.back": "戻る",
  "theme.confirm": "完了",
  "theme.lightStart": "ライト開始",
  "theme.lightEnd": "ライト終了",
  "lang.heading": "言語",
  "lang.hint": "UI 言語を選ぶと次回も覚えています。",
  "lang.switched": "{label} に切り替えました",
  "lang.filterPlaceholder": "言語を絞り込み…",
  "lang.filterEmpty": "一致する言語がありません",
  "nav.about": "について",
  "nav.archive": "アーカイブ",
  "nav.friends": "友人",
  "nav.tags": "タグ",
  "search.open": "検索を開く",
  "search.title": "サイト内検索",
  "search.placeholder": "記事やページを検索…",
  "search.empty": "一致する結果がありません",
  "search.clear": "クリア",
  "search.noIndex": "検索インデックス未準備（先に vp run build）",
  "search.close": "閉じる",
  "about.title": "について",
  "about.body": "vv は vvangh の個人サイト。技術・生活・趣味を残し、構築過程も公開します。",
  "archive.title": "アーカイブ",
  "archive.empty": "公開記事はまだありません。",
  "friends.title": "友人リンク",
  "friends.empty": "まだリンクがありません。",
  "tags.title": "タグ",
  "tags.empty": "まだタグがありません。",
  "tags.tagTitle": "タグ：{tag}",
  "tags.tagIntro": "「{tag}」タグの記事と建設日誌。",
  "splash.skip": "スキップ",
  "splash.brand": "vv",
  "toc.title": "目次",
  "progress.label": "読書進捗",
  "comments.title": "コメント",
  "onThisDay.title": "On this day",
  "onThisDay.hint": "Today is {date}: notes from past years.",
  "onThisDay.empty": "Nothing from prior years on this day.",
  "heatmap.title": "Writing heatmap",
  "heatmap.hint": "Publishing days over the past year (tech + build log).",
  "heatmap.less": "Less",
  "heatmap.more": "More",
  "revisions.title": "Revision timeline",
  "readAloud.minutes": "About {n} min read",
  "readAloud.start": "Read aloud",
  "readAloud.stop": "Stop",
  "readAloud.unsupported": "Speech synthesis is unavailable here.",
  "series.label": "Series",
  "series.prev": "Previous",
  "series.next": "Next",
  "related.title": "関連記事",
  "life.intro": "Diaries, film frames, and places.",
  "life.kindDiary": "Diary",
  "life.kindPhoto": "Film",
  "film.title": "Film gallery",
  "film.hint": "Use left/right arrows; captions are announced.",
  "film.prev": "Previous frame",
  "film.next": "Next frame",
  "travel.title": "Travel map",
  "travel.hint": "City-level footprints (privacy-blurred).",
  "cite.label": "Cite this post",
  "cite.copy": "Copy citation",
  "cite.copied": "Copied",
  "webmentions.title": "Mentions",
  "webmentions.empty": "No webmentions yet.",
  "webmentions.disabled": "Webmention endpoint not configured (see docs).",
  "nowPlaying.listening": "Listening",
  "nowPlaying.watching": "Watching",
  "nowPlaying.empty": "Nothing listed right now.",
  "lab.title": "Repo dashboard",
  "lab.hint": "Engineering health at a glance (static build-time data).",
  "nav.lab": "Lab",
  "lighthouse.title": "Lighthouse wall",
  "lighthouse.hint": "Perf and a11y scores across milestones.",
  "stack.title": "Today’s stack",
  "adr.title": "ADR graph",
  "adr.hint": "Which decisions superseded which.",
  "a11y.density": "Reading density",
  "a11y.densityHint": "Comfort mode enlarges type and line height.",
  "a11y.comfortable": "Comfort",
  "a11y.default": "Default",
  "a11y.contrast": "High contrast",
  "a11y.contrastOn": "Enable high contrast",
  "a11y.contrastOff": "Disable high contrast",
  "a11y.helpTitle": "Keyboard & screen reader tips",
  "a11y.helpOpen": "Open shortcut help",
  "a11y.helpClose": "Close",
  "a11y.helpBody":
    "Press ? for this panel. Ctrl/Cmd+K searches. Skip link jumps to main. Theme, language, and reading aids live in the bottom-right Settings button.",
  "a11y.newWindow": "（新しいウィンドウで開きます）",
  "newsletter.title": "Newsletter",
  "newsletter.hint": "Email updates beyond RSS (form endpoint required).",
  "newsletter.email": "Email",
  "newsletter.submit": "Subscribe",
  "newsletter.privacy": "Used only for updates; unsubscribe anytime.",
  "newsletter.disabled": "Subscribe endpoint missing — see docs/接入/邮件订阅.md.",
  "friends.intro": "ご縁に感謝：愛読しているサイトたちです。",
  "friends.applyTitle": "相互リンク",
  "friends.applyBody": "オリジナル・HTTPS・長期運営——三つ揃えば、リポジトリの Issue でどうぞ。",
  "friends.applyCta": "GitHub で申請",
  "fun.intro": "コード以外の小さなコレクション：ゲーム、プレイリスト、フィルム。",
  "fun.gameTitle": "ミニゲーム",
  "fun.gameDesc": "開いてすぐ遊べるブラウザゲーム、インタラクティブ島を予定。",
  "fun.musicTitle": "今日のプレイリスト",
  "fun.musicDesc": "右下の Now Playing、最近のヘビロテ。",
  "fun.filmTitle": "フィルムアルバム",
  "fun.filmDesc": "フィルムで切り取った日常。",
  "fun.collectTitle": "ブックマーク",
  "fun.collectDesc": "ネットで拾った素敵・面白い・便利。",
  "fun.quoteTitle": "今日の一句",
  "fun.quoteText": "毎日を具体的に生きることが、虚無への唯一の答え。",
  "about.nowTitle": "いま",
  "about.nowRead": "読んでいる：技術ライティングとリファクタリングに格闘中。",
  "about.nowWrite": "書いている：「ゼロからイチへ」建設日誌。",
  "about.nowBuild": "いじっている：vv の細部すべて。",
  "about.colophonTitle": "このサイトについて",
  "about.colophonBody": "Astro + Tailwind CSS で静的生成、ソースは GitHub で公開。",
  "about.linksTitle": "リンク",
  "archive.intro": "公開してきたすべての編年史。",
  "archive.statsPosts": "記事",
  "archive.statsLogs": "建設日誌",
  "archive.statsTags": "タグ",
  "archive.statsDays": "投稿日数",
  "archive.yearCount": "{count} 件",
  "lab.projectsTitle": "実験プロジェクト",
  "lab.repoTitle": "リポジトリ・スナップショット",
  "lab.statusLive": "公開中",
  "lab.statusWip": "改良中",
  "lab.statusIdea": "構想",
  "lab.projHeatmap": "執筆ヒートマップ",
  "lab.projHeatmapDesc": "アーカイブのコミット風ヒート、書くほど緑に。",
  "lab.projRelated": "関連記事",
  "lab.projRelatedDesc": "タグとシリーズによる静的レコメンド。",
  "lab.projReadAloud": "読み上げ",
  "lab.projReadAloudDesc": "長文を音声で。",
  "lab.projGames": "ミニゲーム",
  "lab.projGamesDesc": "趣味ページ用のインタラクティブ島。",
  "build.progressTitle": "進捗",
  "build.progressHint": "マイルストーンを公開で進行、すべて記録。",
  "build.latestMilestone": "最新マイルストーン",
  "posts.count": "{count} 件",
  "blog.intro": "技術エッセイ：ツール、フロントエンド、構築。",
  "tags.intro": "すべてのタグ、頻度順。",
};
