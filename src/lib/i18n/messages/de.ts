/**
 * UI 文案：de。
 */
import type { Catalog } from "./keys";

export const de: Catalog = {
  "nav.home": "Start",
  "nav.blog": "Technik",
  "nav.build": "Von Null",
  "nav.life": "Leben",
  "nav.fun": "Spaß",
  "nav.more": "Mehr",
  "nav.main": "Hauptnavigation",
  "skip.toMain": "Zum Inhalt springen",
  "footer.rights": "© {year} vvangh",
  "footer.license": "Apache-2.0 · Texte © Autor",
  "home.intro":
    "vv ist vvanghs persönliche Seite: Technik, Leben, etwas Unterhaltung—und ein offenes Baulogbuch.",
  "home.latest": "Zuletzt",
  "home.viewAll": "Alle ansehen",
  "home.eyebrow": "vvangh",
  "home.lead": "Technik und Leben, langsam geschrieben—mit Luft dazwischen.",
  "home.scrollCue": "Weiter",
  "bg.prev": "Vorheriger Hintergrund",
  "bg.next": "Nächster Hintergrund",
  "bg.switcher": "Hintergrund wechseln",
  "about.prefs": "Einstellungen",
  "about.prefsHint": "Theme und Sprache — über die Einstellungen unten rechts.",
  "prefs.open": "Einstellungen öffnen",
  "prefs.close": "Schließen",
  "top.label": "Nach oben",
  "blog.title": "Technik",
  "blog.empty": "Technikbeiträge folgen in einem späteren Meilenstein.",
  "build.title": "Von Null",
  "build.empty": "Baulogbuch-Einträge erscheinen hier später.",
  "life.title": "Leben",
  "life.empty": "Tagebuch und Medien folgen später.",
  "fun.title": "Spaß",
  "fun.empty": "Unterhaltung folgt später.",
  "theme.heading": "Erscheinungsbild",
  "theme.hint": "Hell, dunkel, System oder eigener Zeitplan.",
  "theme.scheduleTitle": "Eigener Zeitplan",
  "theme.back": "Zurück",
  "theme.confirm": "Fertig",
  "theme.lightStart": "Hell ab",
  "theme.lightEnd": "Hell bis",
  "lang.heading": "Sprache",
  "lang.hint": "UI-Sprache wählen; wir merken sie uns.",
  "lang.switched": "Gewechselt zu {label}",
  "lang.filterPlaceholder": "Sprachen filtern…",
  "lang.filterEmpty": "Keine passende Sprache",
  "nav.about": "Über",
  "nav.archive": "Archiv",
  "nav.friends": "Freunde",
  "nav.tags": "Tags",
  "search.open": "Suche öffnen",
  "search.title": "Website durchsuchen",
  "search.placeholder": "Beiträge und Seiten suchen…",
  "search.empty": "Keine Treffer",
  "search.clear": "Löschen",
  "search.noIndex": "Suchindex fehlt (zuerst vp run build)",
  "search.close": "Schließen",
  "about.title": "Über",
  "about.body":
    "vv ist vvanghs persönliche Seite: Technik, Leben, etwas Unterhaltung—und ein offenes Baulogbuch.",
  "archive.title": "Archiv",
  "archive.empty": "Noch keine Beiträge.",
  "friends.title": "Freunde",
  "friends.empty": "Noch keine Links.",
  "tags.title": "Tags",
  "tags.empty": "Noch keine Tags.",
  "tags.tagTitle": "Tag: {tag}",
  "tags.tagIntro": "Beiträge und Build-Logs mit Tag „{tag}“.",
  "splash.skip": "Überspringen",
  "splash.brand": "vv",
  "toc.title": "Inhalt",
  "progress.label": "Lesefortschritt",
  "comments.title": "Kommentare",
  "onThisDay.title": "On this day",
  "onThisDay.hint": "Today is {date}: notes from past years.",
  "onThisDay.empty": "Nothing from prior years on this day.",
  "heatmap.title": "Writing heatmap",
  "heatmap.hint": "Publishing days over the past year (tech + build log).",
  "heatmap.less": "Less",
  "heatmap.more": "More",
  "revisions.title": "Revision timeline",
  "readAloud.minutes": "About {n} min read",
  "readAloud.start": "Read aloud",
  "readAloud.stop": "Stop",
  "readAloud.unsupported": "Speech synthesis is unavailable here.",
  "series.label": "Series",
  "series.prev": "Previous",
  "series.next": "Next",
  "related.title": "Ähnliche Beiträge",
  "life.intro": "Diaries, film frames, and places.",
  "life.kindDiary": "Diary",
  "life.kindPhoto": "Film",
  "film.title": "Film gallery",
  "film.hint": "Use left/right arrows; captions are announced.",
  "film.prev": "Previous frame",
  "film.next": "Next frame",
  "travel.title": "Travel map",
  "travel.hint": "City-level footprints (privacy-blurred).",
  "cite.label": "Cite this post",
  "cite.copy": "Copy citation",
  "cite.copied": "Copied",
  "webmentions.title": "Mentions",
  "webmentions.empty": "No webmentions yet.",
  "webmentions.disabled": "Webmention endpoint not configured (see docs).",
  "nowPlaying.listening": "Listening",
  "nowPlaying.watching": "Watching",
  "nowPlaying.empty": "Nothing listed right now.",
  "lab.title": "Repo dashboard",
  "lab.hint": "Engineering health at a glance (static build-time data).",
  "nav.lab": "Lab",
  "lighthouse.title": "Lighthouse wall",
  "lighthouse.hint": "Perf and a11y scores across milestones.",
  "stack.title": "Today’s stack",
  "adr.title": "ADR graph",
  "adr.hint": "Which decisions superseded which.",
  "a11y.density": "Reading density",
  "a11y.densityHint": "Comfort mode enlarges type and line height.",
  "a11y.comfortable": "Comfort",
  "a11y.default": "Default",
  "a11y.contrast": "High contrast",
  "a11y.contrastOn": "Enable high contrast",
  "a11y.contrastOff": "Disable high contrast",
  "a11y.helpTitle": "Keyboard & screen reader tips",
  "a11y.helpOpen": "Open shortcut help",
  "a11y.helpClose": "Close",
  "a11y.helpBody":
    "Press ? for this panel. Ctrl/Cmd+K searches. Skip link jumps to main. Theme, language, and reading aids live in the bottom-right Settings button.",
  "a11y.newWindow": " (öffnet in neuem Fenster)",
  "newsletter.title": "Newsletter",
  "newsletter.hint": "Email updates beyond RSS (form endpoint required).",
  "newsletter.email": "Email",
  "newsletter.submit": "Subscribe",
  "newsletter.privacy": "Used only for updates; unsubscribe anytime.",
  "newsletter.disabled": "Subscribe endpoint missing — see docs/接入/邮件订阅.md.",
  "friends.intro": "Gute Bekannte: Seiten, die ich lese und gern empfehle.",
  "friends.applyTitle": "Linktausch",
  "friends.applyBody":
    "Originalinhalte, HTTPS, langfristige Pflege — wenn das passt, öffne ein Issue zum Tauschen.",
  "friends.applyCta": "Auf GitHub bewerben",
  "fun.intro": "Eine kleine Sammlung jenseits des Codes: Spiele, Playlists und Film.",
  "fun.gameTitle": "Minispiele",
  "fun.gameDesc": "Browserspiele zum sofortigen Loslegen — eine interaktive Insel im Entstehen.",
  "fun.musicTitle": "Heutige Playlist",
  "fun.musicDesc": "Now Playing unten rechts — zuletzt in Dauerschleife.",
  "fun.filmTitle": "Filmalbum",
  "fun.filmDesc": "Auf Film festgehaltene Alltagsmomente.",
  "fun.collectTitle": "Lesezeichen",
  "fun.collectDesc": "Schönes, Verspieltes und Nützliches aus dem Web.",
  "fun.quoteTitle": "Zitat des Tages",
  "fun.quoteText": "Jeden Tag konkret zu leben ist die einzige Antwort auf die Leere.",
  "about.nowTitle": "Jetzt",
  "about.nowRead": "Lese: ringe mit technischem Schreiben und Refactoring.",
  "about.nowWrite": "Schreibe: das Build-Log „Von null auf eins“.",
  "about.nowBuild": "Bastle: an jedem Detail von vv.",
  "about.colophonTitle": "Kolophon",
  "about.colophonBody": "Statisch generiert mit Astro + Tailwind CSS; Open Source auf GitHub.",
  "about.linksTitle": "Anderswo",
  "archive.intro": "Eine Chronik von allem, was hier erschienen ist.",
  "archive.statsPosts": "Beiträge",
  "archive.statsLogs": "Build-Logs",
  "archive.statsTags": "Tags",
  "archive.statsDays": "Tage mit Posts",
  "archive.yearCount": "{count} Einträge",
  "lab.projectsTitle": "Experimente",
  "lab.repoTitle": "Repo-Schnappschuss",
  "lab.statusLive": "Live",
  "lab.statusWip": "In Arbeit",
  "lab.statusIdea": "Idee",
  "lab.projHeatmap": "Schreib-Heatmap",
  "lab.projHeatmapDesc": "Commit-artige Hitze im Archiv — wird täglich grüner.",
  "lab.projRelated": "Ähnliche Beiträge",
  "lab.projRelatedDesc": "Statische Relevanz aus Tags und Serien.",
  "lab.projReadAloud": "Vorlesen",
  "lab.projReadAloudDesc": "Lange Texte, vorgelesen.",
  "lab.projGames": "Minispiele",
  "lab.projGamesDesc": "Eine interaktive Insel für die Fun-Seite.",
  "build.progressTitle": "Fortschritt",
  "build.progressHint": "Meilensteine öffentlich ausgeliefert, jeder dokumentiert.",
  "build.latestMilestone": "Neuester Meilenstein",
  "posts.count": "{count} Beiträge",
  "blog.intro": "Tech-Schreiben: Werkzeuge, Frontend und Bauen.",
  "tags.intro": "Alle Tags, nach Häufigkeit sortiert.",
};
